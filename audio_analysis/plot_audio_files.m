...
for file_i = 1 : length(files)
    ...
    if(bool_perform(18))
        ...
        mir_entropy= mirentropy(mir_spec,'Center');
        mir_entropy_data = mirgetdata(mir_entropy);
        ...
        set(gca, 'FontSize', 12)
        set(gca, 'FontWeight', 'bold', 'FontSize', 14), 
        title([{file_name, 'Feature: Entropy (Centered, HWR)'}], 'FontWeight', 'bold', 'FontSize', 14)

        xlabel({'Time', '(Seconds)'}, 'FontWeight', 'bold', 'FontSize', 14);
        ylabel('Coefficient Value', 'FontWeight', 'bold', 'FontSize', 14);
        ylim([0 1.0]),

        print(h,'-dpdf',sprintf('-r%d',dpi),[output_directory '/' files(file_i).name(1:end-4), '_entropy_hwr.pdf']);
    end
    ...
    if(bool_perform(19))
        ...
        mir_regularity= mirharmonicity(mir_spec);
        mir_regularity_data = mirgetdata(mir_regularity);
