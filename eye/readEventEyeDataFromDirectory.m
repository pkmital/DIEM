...
function [left_x left_y left_dil left_flag right_x right_y right_dil right_flag] = ...
    readEventEyeDataFromDirectory(directory, movie_name, not_movie_name)
...
for i =  1 : num_files
    ...
    if(~isempty(strfind(current_file,movie_name)) && isempty(strfind(current_file, not_movie_name)) || ...
       ~isempty(strfind(lower(current_file),lower(movie_name))) && isempty(strfind(current_file, not_movie_name)) )
        count = count + 1;
...
if count > 0
    ...
    fprintf('No eye-movement files found for %s!\n', movie_name);
    pause;
    return;
end
