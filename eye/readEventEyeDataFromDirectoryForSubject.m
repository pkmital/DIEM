% Parag K. Mital
% DIEM 2009
function [left_x left_y left_dil left_flag right_x right_y right_dil right_flag] = ...
    readEventEyeDataFromDirectoryForSubject(directory, movie_name, subject_name)
...
for i =  1 : num_files
    ...
    if(~isempty(strfind(current_file,movie_name)) && ~isempty(strfind(current_file, subject_name)) || ...
       ~isempty(strfind(lower(current_file),lower(movie_name))) && ~isempty(strfind(current_file, subject_name)) )
        count = count + 1;
...
if count > 0
    % fprintf('Read %d files for the movie: %s\n', count, movie_name);
else
    fprintf('No eye-movement files found for movie %s and subject %s!\n', movie_name, subject_name);
    return;
end
