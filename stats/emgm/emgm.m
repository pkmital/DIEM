function [label, model, llh] = emgm(X, init, cov_type)
% Perform EM algorithm for fitting the Gaussian mixture model.
%
%   X: d x n data matrix
%   init: k (1 x 1) or label (1 x n, 1<=label(i)<=k) or center (d x k)
%   (optional) cov_type = 'full' (default), 'diag', 'spherical'
%
% Extension of EM- GMM for different covariance types by
% Parag K Mital, 2011
% http://pkmital.com (parag@pkmital.com).
%
% Original code by Michael Chen (sth4nth@gmail.com).
%
%% initialization
...
while ~converged && t < maxiter
    t = t+1;
    model = maximization(X,R,cov_type,tol);
...
function model = maximization(X, R, cov_type, tol)
...
for i = 1:k
   ...
   switch cov_type,
     case 'full', 
       Sigma(:,:,i) = Xo*Xo'/s(i);
     case 'diag', 
       Sigma(:,:,i) = diag(sum(Xo.^2,2)/s(i));
     case 'spherical'
       Sigma(:,:,i) = eye(d,d)*sum(sum(Xo.^2))/(s(i)*d);
     otherwise
       error('Wrong cov_type.');
   end
   
   Sigma(:,:,i) = Sigma(:,:,i)+eye(d)*tol; % add a prior for numerical stability
end
